...
def parse_uploaded_csv(contents: str) -> pd.DataFrame:
    if not contents:
        raise ValueError("Upload the USAFA Academic Calendar CSV first.")
    try:
        _, encoded = contents.split(",", 1)
        decoded = base64.b64decode(encoded)
        return pd.read_csv(io.BytesIO(decoded), encoding="utf-8-sig")
    except Exception as exc:
        raise ValueError("The uploaded file could not be read as a CSV.") from exc
...
def filter_academic_rows(calendar_df: pd.DataFrame, academic_mode: str):
    ...
    academic_mode values:
      ...
      - major: uploaded rows except M/T rotation markers such as M1 and T2
      - all: every row in the uploaded CSV
    """
    if academic_mode == "none":
        return calendar_df.iloc[0:0].copy()
    if academic_mode == "major":
        mask = ~calendar_df["Subject"].fillna("").astype(str).str.strip().str.fullmatch(r"[MT]\d+")
        return calendar_df.loc[mask].copy()
...
def field(label, component, width="220px"):
    return html.Div([html.Label(label), component], style={"display": "inline-block", "verticalAlign": "top", "marginRight": "12px", "marginBottom": "8px", "width": width})


def make_course_row(index):
    return html.Div([
        html.H5(f"Course {index + 1}", style={"margin": "0 0 8px 0"}),
        field("Course name", dcc.Input(id={"type": "course-name", "index": index}, type="text", placeholder="e.g., Math 356", style={"width": "100%"})),
        ...
        field("Location", dcc.Input(id={"type": "course-location", "index": index}, type="text", placeholder="e.g., Fairchild Hall 2E17", style={"width": "100%"})),
        html.Br(),
        field("Description", dcc.Input(id={"type": "course-description", "index": index}, type="text", placeholder="Optional", style={"width": "100%"}), width="520px"),
        field("Category", dcc.Input(id={"type": "course-category", "index": index}, type="text", value="Teaching", style={"width": "100%"}), width="180px"),
    ], style={"border": "1px solid #ddd", "borderRadius": "6px", "padding": "12px", "marginBottom": "10px", "maxWidth": "980px"})
...
app.layout = html.Div([
    html.H2("USAFA Teaching Schedule Generator"),
    html.P("Upload the USAFA Academic Calendar CSV, select the courses and periods you teach, then download an Outlook import file."),
    dcc.Upload(
        id="calendar-upload",
        children=html.Div(["Drag and drop or select the Academic Calendar CSV"]),
        style={"border": "1px dashed #999", "padding": "20px", "maxWidth": "650px", "textAlign": "center"},
        multiple=False,
    ),
    html.Div(id="upload-status", style={"margin": "10px 0 18px 0"}),
    html.H4("Courses"),
    html.Div(id="course-rows", children=[make_course_row(0)]),
    html.Button("Add course row", id="add-row", n_clicks=0),
    html.H4("Calendar options", style={"marginTop": "22px"}),
    html.Label("Include uploaded USAFA Academic Calendar events in the export"),
    dcc.RadioItems(
        id="academic-export-mode",
        options=[
            {"label": " Teaching appointments only", "value": "none"},
            {"label": " Teaching + major academic calendar events (exclude M/T rotation labels)", "value": "major"},
            {"label": " Teaching + every uploaded CSV event (include M/T rotation labels)", "value": "all"},
        ],
        value="none",
    ),
    html.Br(),
    html.Label("Reminder for teaching appointments"),
    dcc.RadioItems(id="reminder-on", options=[{"label": "No reminder", "value": "no"}, {"label": "Reminder", "value": "yes"}], value="yes", inline=True),
    html.Br(),
    html.Label("Reminder minutes before class"),
    dcc.Input(id="reminder-minutes", type="number", value=15, min=0, step=5),
    html.Br(), html.Br(),
    html.Label("Show time as"),
    dcc.Dropdown(id="busy-status", options=[{"label": "Free", "value": "0"}, {"label": "Tentative", "value": "1"}, {"label": "Busy", "value": "2"}, {"label": "Out of Office", "value": "3"}], value="2", style={"width": "240px"}),
    html.Br(),
    dcc.Checklist(id="private", options=[{"label": "Mark appointments private", "value": "private"}], value=[]),
    html.Br(),
    html.Button("Preview events", id="preview", n_clicks=0),
    html.Button("Download .ics", id="download-ics", n_clicks=0, style={"marginLeft": "10px"}),
    html.Button("Download Outlook .csv", id="download-csv", n_clicks=0, style={"marginLeft": "10px"}),
    dcc.Download(id="download"),
    html.Div(id="message", style={"marginTop": "16px", "fontWeight": "bold"}),
...
def upload_status(contents, filename):
    if not contents:
        return "No calendar uploaded yet."
    try:
        df = parse_uploaded_csv(contents)
        days, modified = extract_schedule_days(df)
        return f"Loaded {filename}: found {len(days)} M/T class days and {len(modified)} modified SOC day(s)."
    except Exception as exc:
        return f"Upload problem: {exc}"
...
def preview_events(_n_clicks, contents, names, periods, locations, descriptions, categories, academic_mode, reminder_value, reminder_minutes, busy_status, private_values):
    try:
        ...
        table = dash_table.DataTable(data=sample, page_size=20, style_cell={"textAlign": "left"})
